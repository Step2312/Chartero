...
import { React, G, createRoot } from '../global';
...
class PDFImages extends ReaderImages<'pdf'> {
    ...
    private onRenderImage: PdfImageListener = (pageNum, imgNum, { rect, pageIdx, data }) => {
        ...
        try {
            ...
            this.root ??= createRoot(this.imagesView);
    ...
    async loadAllImages() {
        ...
        try {
            ...
            addon.worker.unsubscribePDF(dat.url);
        }
    }
...
> {
    ...
    protected onImageClick(e: MouseEvent): void {
        const idx = Number((e.target as HTMLImageElement).id.split('-').at(-1)) - 1,
            img = this.imageLinks[idx];
    ...
    protected onImageDblClick(this: HTMLImageElement) {
        ...
        (canvas.getContext('2d') as CanvasRenderingContext2D | null)?.drawImage(this, 0, 0);
