...
export function onAddonLoad() {
    ...
    const notifyExcludedTagRulesChanged = () => {
        ...
        addon.log('Updating excluded tag rules');
    };
    addon.addPrefsObserver(notifyExcludedTagRulesChanged, 'excludedTags');
    addon.addPrefsObserver(notifyExcludedTagRulesChanged, 'excludedTagPatterns');
...
export async function onItemSelect() {
    ...
    if ('duplicates' == ZoteroPane.getCollectionTreeRow()?.type || !items.length) {
        ...
        switch (row?.type) {
            ...
            case 'group':
                // case 'feed':
                renderLibrarySummary((row.ref as Zotero.DataObject).libraryID);
                break;
