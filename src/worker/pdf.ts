...
async function getPageImages(pdf: PDFDocumentProxy, pageIdx: number, url: string) {
    const page = await pdf.getPage(pageIdx + 1),
        ...
        datList = await Promise.all(imgList.map(
            opIdx => new Promise<{ dat: ImageBitmap | null, opIdx?: number }>(resolve => {
                const id = opList.argsArray[opIdx][0], timer = setTimeout(() => {
                    console.warn(`Image ${id} timed out in page ${pageIdx + 1}`);
                    resolve({ dat: null });
                }, 10000);
                page.objs.get(id, (dat: any) => {
                    clearTimeout(timer);
                    resolve({ dat: dat.bitmap, opIdx });
                });
            })
        ));

    for (const { dat, opIdx } of datList) {
        if (!dat) continue;
        const matrixes = new Array<Matrix>();
        for (let j = opIdx! - 2; j > 1 && matrixes.length < 5; j -= 2)
        ...
        postMessage(
            {
                stream: {
                    ...
                    payload: { data: dat, id: opIdx, rect, pageIdx },
                } as WorkerStream,
            },
            [dat],
        );
