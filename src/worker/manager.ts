export interface WorkerMethods {
    getAllImages: (url: string) => Promise<number>;
    getPdfDoc: (url: string) => Promise<any>;
    processPDF: (url: string) => Promise<unknown>;
...
export abstract class WorkerManagerBase<T extends WorkerMode> {
    ...
    private onResponse(response: WorkerResponse) {
        const { resolve, reject } = this.queue.pop(response.id);
        if (response.error) {
    ...
    query: T extends Worker ? QuerySlave<keyof WorkerMethods> : QueryMaster = (
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            (this.that.postMessage as (message: unknown) => void)({ request });
        });
    };
}
...
class TaskQueue {
    ...
    pop(id: number) {
        ...
        delete this.data[id];
        if (!task) throw new Error('Unknown worker response id: ' + id);
        return task;
    }
}
